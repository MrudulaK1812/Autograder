...
load_dotenv()
MONGO_URI = os.getenv("STUDENT_DB_URI")  # Securely fetch MongoDB URI from .env
...
# -------------------- Evaluate the Test Section --------------------
    ...
    MISTRAL_API_KEY_EVALUATION = os.getenv("MISTRAL_API_KEY_EVALUATION")
            ...
            if document and "questions" in document:
                ...
                for index, q in enumerate(questions):
                    if "question_number" not in q:
                        q["question_number"] = str(index + 1)  # Default to 1-based index
                return questions
            else:
                return []
        except Exception as e:
    ...
    def evaluate_answers(grouped_answers, questions_data, student_name, prn, test_id):
        ...
        for i, question in enumerate(questions_data):
            ...
            evaluation_result = ""  # Initialize here

            if score is None:
                prompt = f"""
                    As an expert educational assessor, evaluate the following student answer to the question with EXTREME leniency and provide a score between 0 and 5 (integer only). Even for a minimal attempt, the score should be at least 3. If the answer shows some understanding, aim for 4 or 5. Only if the answer is completely blank or irrelevant should the score be 0, 1, or 2.

                    QUESTION: {q_text}

                    STUDENT ANSWER: {student_answer}

                    Consider any relevant keywords: {', '.join(keywords)}.

                    Respond with ONLY the numeric score (an integer between 0 and 5) followed by a brief encouraging feedback (2-3 sentences) separated by a newline.
                    For example:
                    4
                    Good attempt! You touched upon the key concepts...
                    """
                ...
                try:
                    ...
                    st.markdown(f"**Raw Evaluation Response (Question {q_num}):** {evaluation_result}") # Debugging
                except Exception as e:
                    st.error(f"Error evaluating question {q_num}: {str(e)}")
                    evaluation_result = "0\nError during evaluation."
                    score = 0

                # Extract score (expecting it on the first line)
                lines = evaluation_result.split('\n')
                if lines:
                    try:
                        raw_score = int(lines[0].strip())
                        score = max(0, min(5, raw_score))
                        evaluation_result = "Feedback: " + "\n".join(lines[1:]).strip()
                    except ValueError:
                        score = 0
                        evaluation_result = f"Feedback: Could not parse score from response: '{evaluation_result}'"
                else:
                    score = 0
                    evaluation_result = "Feedback: No response received for evaluation."
